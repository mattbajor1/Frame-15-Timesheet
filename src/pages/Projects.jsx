...
export default function Projects({ email }) {
  ...
  const filtered = useMemo(() => {
    ...
    return projects.filter((p) =>
      `${p.number} ${p.name} ${p.client} ${p.status}`.toLowerCase().includes(s)
    );
  }, [projects, q]);
  ...
  return detail ? (
    ...
    <div className="space-y-4">
      <div className="flex gap-2">
        ...
        />
        <button className="f15-btn f15-btn--primary" onClick={openModal}>
          New project
        </button>
      </div>

      <div className="f15-card">
        <div className="f15-h2 mb-3">
          Projects <span className="text-neutral-400 text-sm">({filtered.length})</span>
        </div>
        {filtered.length === 0 ? (
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center p-4">
          <div className="f15-card w-full max-w-lg space-y-3">
            ...
            <div className="flex justify-end gap-2">
              <button className="f15-btn" onClick={() => setModal(false)} disabled={saving}>
                Cancel
              </button>
              <button
                className="f15-btn f15-btn--primary"
                onClick={submit}
                disabled={saving}
              >
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        <button className="f15-btn" onClick={onBack}>
          ← Back
        </button>
      ...
      <div className="f15-card grid md:grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <input
            className="f15-input"
            value={edit.client}
            onChange={(e) => setEdit((v) => ({ ...v, client: e.target.value }))}
          />
          ...
          >
            {["Planning", "Active", "On Hold", "Done"].map((s) => (
              <option key={s} value={s}>
                {s}
              </option>
            ))}
        ...
        <div className="space-y-2">
          ...
          <input
            ...
            onChange={(e) => setEdit((v) => ({ ...v, startDate: e.target.value }))}
          />
          ...
          <input
            ...
            type="date"
            value={edit.dueDate || ""}
            onChange={(e) => setEdit((v) => ({ ...v, dueDate: e.target.value }))}
          />
          <label className="text-xs text-neutral-400">Budget (hours)</label>
          <input
            className="f15-input"
            type="number"
            value={edit.budgetHours || 0}
            onChange={(e) =>
              setEdit((v) => ({ ...v, budgetHours: Number(e.target.value || 0) }))
            }
          />
        </div>
        ...
        <div className="md:col-span-2 flex justify-end">
          <button className="f15-btn f15-btn--primary" onClick={saveMeta}>
            Save changes
          </button>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <div className="f15-card lg:col-span-2">
          ...
          <form onSubmit={quickAddTask} className="flex gap-2 mb-3">
            ...
            />
            <button className="f15-btn f15-btn--blue" disabled={adding}>
              {adding ? "Adding…" : "Add"}
            </button>
          </form>
                  ...
                  <div className="text-xs text-neutral-400 mt-1 flex gap-4 items-center">
                    ...
                    <div className="ml-auto">
                      <span className="text-xs px-2 py-1 rounded-full border">
                        {(detail.time.byTask.find((x) => x.task === t.name)?.hours || 0).toFixed(2)}{" "}
                        h
                      </span>
                    </div>
                  </div>
      ...
      {days.map((d) => {
        ...
        return (
          <div key={k} className="f15-tile">
            ...
            {inRange ? null : (
              <div className="text-[10px] text-neutral-500 mt-1">outside project</div>
            )}
            {items.length > 0 && (
